...
const constants = { 'RSA_NO_PADDING': 3 };
// require ('constants');
...
module.exports.makeScheme = function (key, options) {
    ...
    Scheme.prototype.sign = function (buffer) {
        ...
        if (this.options.environment === 'browser') {
            ...
            const hasher = CryptoJS[hashAlgorithm.toUpperCase ()] (buffer.toString ());
            const asBuffer = wordArrayToBuffer (hasher);
            const paddedHash = this.pkcs1pad (asBuffer, hashAlgorithm);
            const res = this.key.$doPrivate (new BigInteger (paddedHash)).toBuffer (this.key.encryptedDataLength);
            return res;
        } else {
