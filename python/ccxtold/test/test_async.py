...
import ccxtold.async_support as ccxt  # noqa: E402
...
path = os.path.dirname(ccxtold.__file__)
if 'site-packages' in os.path.dirname(ccxtold.__file__):
...
async def try_all_proxies(exchange, proxies=['']):
    ...
    for num_retries in range(0, max_retries):
        ...
        except (ccxtold.RequestTimeout, ccxtold.AuthenticationError, ccxtold.NotSupported, ccxtold.DDoSProtection, ccxtold.ExchangeNotAvailable, ccxtold.ExchangeError) as e:
...
for id in ccxtold.tealstreet_exchanges:
    if id == 'theocean':
        continue
    ...
    if id in config:
        exchange_config = ccxtold.Exchange.deep_extend(exchange_config, config[id])
