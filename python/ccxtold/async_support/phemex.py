...
from ccxtold.base.decimal_to_precision import ROUND
from ccxtold.base.decimal_to_precision import DECIMAL_PLACES
from ccxtold.base.decimal_to_precision import TICK_SIZE
from ccxtold.phemex import PhemexTealstreetMixin
...
class phemex(PhemexTealstreetMixin, Exchange):
    ...
    async def fetch_markets(self, params={}):
        ...
        for i in range(0, len(products)):
            ...
            if type == 'perpetual':
                ...
                initialMaintenanceMargin = min([riskLimit['initialMargin'] for riskLimit in riskLimitValues['riskLimits']])
                maxLeverage = None
                if initialMaintenanceMargin:
                    leverageOptions = [x['options'] for x in leverages if x['initialMargin'] == initialMaintenanceMargin][0]
    ...
    # TODO: TEALSTREET is this needed?
    def from_en(self, en, scale, precision=None, precisionMode=None):
        if precision:
            return self.from_en2(en, scale, precision, precisionMode)
        if en is None:
            return None
        precise = Precise(str(en))
        precise.decimals = self.sum(precise.decimals, scale)
        precise.reduce()
        return float(str(precise))
    ...
    def from_en2(self, en, scale, precision, precisionMode=None):
        if en is None:
            return en
        ...
        if isinstance(trade, list):
            ...
            if feeCost is not None:
                ...
                if feeRateEr < 0:
                    ...
                    feeRate = self.parse_number(self.from_er(feeRateEr, market))
