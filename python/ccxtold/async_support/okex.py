...
from ccxtold.base.decimal_to_precision import TRUNCATE
...
class okex(OkexTealstreetMixin, Exchange):
    ...
    def parse_swap_balance(self, response):
        ...
        balance['marginMode'] = self.fetch_margin_mode()
        return balance
    ...
    async def fetch_balance(self, params={}):
        ...
        result['datetime'] = self.iso8601(timestamp)
        balance = self.parse_balance(result, False)
        balance['marginMode'] = await self.fetch_margin_mode() # TEALSTREET
        return balance
    ...
    def parse_order(self, order, market=None):
        ...
        triggerTime = self.safe_string(order, 'triggerTime')
        if triggerTime:
            status = 'closed'
        ...
        clientOrderId: str = self.safe_string(order, 'clOrdId')
        if (clientOrderId is not None) and (len(clientOrderId) < 1):
            clientOrderId = None  # fix empty clientOrderId string
        reduce = self.safe_boolean(order, 'reduceOnly')
        posSide = self.safe_string(order, 'posSide')
        close = self.safe_boolean(order, 'closeOnTrigger') or reduce
        if posSide == 'sell' and side == 'buy':
            close = True
            reduce = True
        elif posSide == 'buy' and side == 'sell':
            close = True
            reduce = True
        if clientOrderId and clientOrderId.endswith('r1'):
            close = True
            reduce = True
        marginType = self.safe_string(order, 'tdMode')
        trigger = self.safe_string(order, 'slTriggerPxType') or self.safe_string(order, 'tpTriggerPxType') or ''
        try:
