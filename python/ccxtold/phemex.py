...
class PhemexTealstreetMixin(object):
    def parse_position(self: 'phemex', position, contract_type, account_balance):
        ...
        id = symbol # + ':' + side
        rawContracts = self.safe_integer(position, 'size')
        contracts = rawContracts * (1 if side == 'long' else -1)
        ...
        maintenanceMargin = float(Precise.string_div(self.safe_string(position, 'posCostEv'), evScale))
